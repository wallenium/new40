...
myDF2$usage <- as.numeric(as.character(myDF2$usage))
myDF3 <- myDF2

myDF2 <- myDF3 %>%
  group_by(id) %>%
  mutate(diff = usage - lag(usage, default = first(usage), order_by = time))
debug_rohdaten <- myDF2 %>% group_by(month=floor_date(time, "month"), id) %>% summarize(amount=last(usage))
...
data_newtime_consumption$consumption <- as.numeric(as.numeric(as.character(data_newtime_consumption$usage_end)) - as.numeric(as.character(data_newtime_consumption$usage_start)))
debug_newtime <- data_newtime_consumption %>% group_by(month=floor_date(date_start, "month"), id) %>% summarize(amount=sum(consumption))
#debug_newtime2 <- data_new_time %>% group_by(month=floor_date(time, "month"), id) %>% summarize(amount=last(usage))

#calculate 
data_consumption_per_day <- myDF2 %>% group_by(month=floor_date(time, "day"), id) %>% summarize(total_consumption=sum(diff))
...
debug_ootime <- data_new_oot_sum %>% group_by(month=floor_date(day, "month"), id) %>% summarize(amount=sum(consumption))
debug_ootime2 <- data_new_oot %>% group_by(month=floor_date(time, "month"), id) %>% summarize(amount=last(usage))
...
data_new_it_sum <- data_newtime_consumption %>% group_by(day=floor_date(date_start, "day"), id) %>% summarize(Billig=sum(consumption))
debug_newit <- data_new_it_sum %>% group_by(month=floor_date(day, "month"), id) %>% summarize(Billig=sum(Billig))
...
names(data_new_oot2_sum)[3] <- "Teuer"

consumptionPerDay <- merge(data_new_it_sum, data_new_oot2_sum, by = c("day", "id"), all = TRUE)
consumptionPerDay[is.na(consumptionPerDay)] <- 0
#debug_cpd <- consumptionPerDay %>% group_by(month=floor_date(day, "month"), id) %>% summarize(Billig=sum(Billig), Teuer=sum(Teuer))
...
# save
write.csv(x = consumptionPerDay, file = "~/Documents/Programmierung/R-Scripts/New40/exported_data.csv")
